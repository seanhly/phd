...
def set_region_and_public_ipv4(host: str, region: str, public_ipv4: str) -> Popen:
	return ssh_do(
		host,
		f"{REDIS_CLI_BINARY} -n {REDIS_WORKER_NETWORK_DB}",
		stdin=[
...
def get_region(host: str = None) -> str:
	from redis import Redis
	return Redis(host=host, db=REDIS_WORKER_NETWORK_DB).get("region").decode().strip()

def get_public_ipv4() -> str:
	from redis import Redis
	return Redis(db=REDIS_WORKER_NETWORK_DB).get("public-ipv4").decode().strip()