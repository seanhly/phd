...
class DistributeSemanticScholarCorpus(UserAction):
	...
	def description(cls):
		return (
			"Distribute the Semantic Scholar corpus' files around the worker network"
		)
	...
	def execute(self) -> None:
		...
		for root, _, candidates in walk(".", topdown=False):
			for file in candidates:
				if file.endswith(".jsonl.gz"):
					...
					m = re.match(
						".*/(pdf_parses|metadata)/(pdf_parses|metadata)_["
						"0-9]+\\.jsonl\\.gz",
						file_path,
					)
					if m:
						...
						bytes_per_device[device_id] = bytes_per_device.get(
							device_id, 0) + compressed_size
						if device_id not in device_roots:
							device_id = stat(file_path).st_dev
							eventual_device_root = file_path
							while True:
								candidate_device_path = "/".join(
									eventual_device_root.split("/")[:-1])
								if not candidate_device_path:
									if stat("/").st_dev == device_id:
										device_roots[device_id] = "/"
									else:
										device_roots[
											device_id] = eventual_device_root
									break
								elif stat(
										candidate_device_path).st_dev == device_id:
						...
						if m[1] == "metadata":
							...
							if not data_compression_ratio_found:
								...
								if ratio < min_data_compression_ratio:
									data_compression_ratio_found = (
										min_data_compression_ratio - ratio
										< 0.001
									)
									print("%.10f" % abs(
										ratio - min_data_compression_ratio))
									min_data_compression_ratio = ratio
		...
		for size, file, is_metadata in files:
			...
			who_gets_what[worker] = who_gets_what.get(worker, []) + [
				(file, is_metadata)]
		...
		for ip, lst in who_gets_what.items():
			...
			outgoing_ss_metadata_for_ip = join(
				rsync_src_dir_for_ip, SS_METADATA
			)
		...
		for ip, lst in who_gets_what.items():
			...
			for data_file in data_files:
				move(
					join(outgoing_ss_data_for_ip, basename(data_file)),
					data_file
				)
